...
export default function Home({
    ...
    useEffect(() => {
        const fetchItems = async () => {
            if (marketplace && nft) {
                ...
                setError("Please connect to your Metamask to view and purchase marketplace items.");
            }
        };
        fetchItems();
    }, [marketplace, nft]);
    return (
        <div className='w-full min-h-[calc(100vh-64px)] bg-gray-100 dark:bg-gray-950 p-4'>
            <p className=' text-xs dark:text-gray-400'>BUY, SELL AND DISCOVER RARE DIGITAL ITEMS</p>
            <h1 className='mt-3 text-2xl font-bold dark:text-white'>Welcome to the NFT Marketplace</h1>
            {error && <div className='flex flex-col gap-2 justify-center items-center h-[calc(100vh-200px)] '>
                <p className='text-red-500 mt-2'>{error}</p>
                {(!marketplace || !nft) && <Button onClick={connectWallet} className='ml-4'>Connect Wallet</Button>}
            </div>}

            {loading ? (
                <div className='flex justify-center items-center h-[calc(100vh-64px)] bg-gray-100 dark:bg-gray-950'>
                    <div className="animate-spin rounded-full h-16 w-16 border-b-2 border-blue-500"></div>
                    <p className='ml-2 text-gray-500 dark:text-gray-400'>Loading Marketplace Items...</p>
                </div>
            ) : items.length === 0 ? (
                ...
                <div className='mt-4'>
                    <p className='text-gray-600 dark:text-gray-400 mb-4'>Explore the available items below:</p>
                    ...
                    {items.map((item, index) => (
                        <div key={index} className='border rounded-lg overflow-hidden shadow-md'>
                            ...
                            <div className='p-4'>
                                ...
                                <Button onClick={() => purchaseItem(item.id)} className='mt-2 font-bold'>Buy for {item.price} ETH</Button>
                            </div>
                        </div>
                    ))}
                </div>
